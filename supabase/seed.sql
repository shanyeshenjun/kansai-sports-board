insert into public.events (
  id, title, sport_type, area, venue_name, address, start_datetime, end_datetime, fee,
...
(
  ...
  '初心者と経験者が混ざってダブルス中心に回します。ラケット貸出は数本あります。', '室内シューズを持参してください。', 'open'
),
(
  'tennoji-basketball',
...
on conflict (id) do update set
  ...
  organizer_contact_type = excluded.organizer_contact_type,
  organizer_contact_value = excluded.organizer_contact_value,
...
insert into public.registrations (
  id, event_id, participant_name, contact, number_of_people, note
) values (
  'reg-1', 'osaka-badminton-umeda', '山田', 'line:yamada', 2, '初心者2名です'
)
on conflict (id) do update set
